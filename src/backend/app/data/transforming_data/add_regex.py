...
def prepare_df_for_regex(dynamic_modifier_df):
    ...
    dynamic_modifier_df.loc[:, "effect"] = dynamic_modifier_df["effect"].str.replace("+", r"\+") # Do we need to do the same for `-`? Can other regex flags occur?
...
def add_regex_column(grouped_dynamic_modifier_df):
    ...
    grouped_dynamic_modifier_df["regex"] = grouped_dynamic_modifier_df.apply(create_regex_string_from_row, axis=1)
...
def return_grouped_df_to_normal(dynamic_modifier_df, grouped_dynamic_modifier_df):
    ...
    dynamic_modifier_df.loc[:, "effect"] = dynamic_modifier_df["effect"].str.replace(r"\+", "+")
...
def main():
    ...
    dynamic_modifier_df, static_modifier_df = divide_modifiers_into_dynamic_static(modifier_df=modifier_df)
    ...
    grouped_dynamic_modifier_df = add_regex_column(grouped_dynamic_modifier_df=grouped_dynamic_modifier_df)
    dynamic_modifier_df = return_grouped_df_to_normal(dynamic_modifier_df=dynamic_modifier_df, grouped_dynamic_modifier_df=grouped_dynamic_modifier_df)
    final_df = combine_dynamic_static(dynamic_modifier_df=dynamic_modifier_df, static_modifier_df=static_modifier_df)
