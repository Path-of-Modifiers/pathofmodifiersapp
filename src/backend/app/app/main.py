...
async def create_currency(
    ...
    return await _crud.CRUD_currency.create(db=db, obj_in=currency)
...
async def get_currency(
    ...
    currency = await _crud.CRUD_currency.get(db=db, filter=currency_map)
    return currency
...
async def get_all_currency(db: _orm.Session = _fastapi.Depends(_deps.get_db)):
    all_currency = await _crud.CRUD_currency.get(db=db)
    return all_currency
...
async def delete_currency(
    ...
    currency_map = {"currencyId": currencyId}
    await _crud.CRUD_currency.remove(filter=currency_map, db=db)
...
async def update_currency(
    ...
    currency_map = {"currencyId": currencyId}
    currency = await _crud.CRUD_currency.get(
        db=db,
        filter=currency_map,
    )

    return await _crud.CRUD_currency.update(
        db_obj=currency, obj_in=currency_update, db=db
    )
...
async def create_account(
    ...
    return await _crud.CRUD_account.create(db=db, obj_in=account)
...
async def get_account(
    ...
    account = await _crud.CRUD_account.get(db=db, filter=account_map)
    return account
...
async def get_all_accounts(db: _orm.Session = _fastapi.Depends(_deps.get_db)):
    all_accounts = await _crud.CRUD_account.get(db=db)
    return all_accounts
...
async def delete_account(
    ...
    account_map = {"accountName": accountName}
    await _crud.CRUD_account.remove(db=db, filter=account_map)
...
async def update_account(
    ...
    account_map = {"accountName": accountName}
    account = await _crud.CRUD_account.get(
        db=db,
        filter=account_map,
    )

    return await _crud.CRUD_account.update(db_obj=account, obj_in=account_update, db=db)
...
async def create_itemBaseType(
    ...
    return await _crud.CRUD_itemBaseType.create(db=db, obj_in=itemBaseType)
...
async def get_itemBaseType(
    ...
    itemBaseType = await _crud.CRUD_itemBaseType.get(db=db, filter=itemBaseType_map)
    return itemBaseType
...
async def get_all_itemBaseTypes(db: _orm.Session = _fastapi.Depends(_deps.get_db)):
    all_ItemBaseTypes = await _crud.CRUD_itemBaseType.get(db=db)
...
async def delete_itemBaseType(
    ...
    itemBaseType_map = {"baseType": baseType}
    await _crud.CRUD_itemBaseType.remove(db=db, filter=itemBaseType_map)
...
async def update_itemBaseType(
    ...
    itemBaseType_map = {"baseType": baseType}
    itemBaseType = await _crud.CRUD_itemBaseType.get(
        db=db,
        filter=itemBaseType_map,
    )

    return await _crud.CRUD_itemBaseType.update(
...
async def create_item(
    ...
    return await _crud.CRUD_item.create(db=db, obj_in=item)
...
async def get_item(itemId: str, db: _orm.Session = _fastapi.Depends(_deps.get_db)):
    ...
    item = await _crud.CRUD_item.get(db=db, filter=item_map)
    return item
...
async def get_all_items(db: _orm.Session = _fastapi.Depends(_deps.get_db)):
    all_items = await _crud.CRUD_item.get(db=db)
    return all_items
...
async def delete_item(itemId: str, db: _orm.Session = _fastapi.Depends(_deps.get_db)):
    item_map = {"itemId": itemId}
    await _crud.CRUD_item.remove(db=db, filter=item_map)
...
async def update_item(
    ...
    item_map = {"itemId": itemId}
    item = await _crud.CRUD_item.get(
        db=db,
        filter=item_map,
    )

    return await _crud.CRUD_item.update(db_obj=item, obj_in=item_update, db=db)
...
async def create_stash(
    ...
    return await _crud.CRUD_stash.create(db=db, obj_in=stash)
...
async def get_stash(stashId: str, db: _orm.Session = _fastapi.Depends(_deps.get_db)):
    ...
    stash = await _crud.CRUD_stash.get(db=db, filter=stash_map)
    return stash
...
async def get_all_stashes(db: _orm.Session = _fastapi.Depends(_deps.get_db)):
    all_stashes = await _crud.CRUD_stash.get(db=db)
    return all_stashes
...
async def delete_stash(stashId: str, db: _orm.Session = _fastapi.Depends(_deps.get_db)):
    stash_map = {"stashId": stashId}
    await _crud.CRUD_stash.remove(db=db, filter=stash_map)
...
async def update_stash(
    ...
    stash_map = {"stashId": stashId}
    stash = await _crud.CRUD_stash.get(
        db=db,
        filter=stash_map,
    )

    return await _crud.CRUD_stash.update(db_obj=stash, obj_in=stash_update, db=db)
...
async def create_modifier(
    ...
    return await _crud.CRUD_modifier.create(db=db, obj_in=modifier)
...
async def get_modifier(
    ...
    modifier = await _crud.CRUD_modifier.get(db=db, filter=modifier_map)
    return modifier
...
async def get_all_modifiers(db: _orm.Session = _fastapi.Depends(_deps.get_db)):
    all_modifiers = await _crud.CRUD_modifier.get(db=db)
...
async def delete_modifier(
    ...
    if position is not None:
        modifier_map["position"] = position
    await _crud.CRUD_modifier.remove(db=db, filter=modifier_map)
...
async def update_modifier(
    ...
    modifier = await _crud.CRUD_modifier.get(
        db=db,
        filter=modifier_map,
    )

    return await _crud.CRUD_modifier.update(
        db_obj=modifier, obj_in=modifier_update, db=db
    )
...
async def create_item_modifier(
    ...
    return await _crud.CRUD_itemModifier.create(db=db, obj_in=itemModifier)
...
async def get_item_modifier(
    ...
    itemModifier = await _crud.CRUD_itemModifier.get(db=db, filter=itemModifier_map)
    return itemModifier
...
async def get_all_item_modifiers(db: _orm.Session = _fastapi.Depends(_deps.get_db)):
    all_itemModifiers = await _crud.CRUD_itemModifier.get(db=db)
...
async def delete_item_modifier(
    ...
    await _crud.CRUD_itemModifier.remove(db=db, filter=itemModifier_map)
...
async def update_item_modifier(
    ...
    itemModifier = await _crud.CRUD_itemModifier.get(
        db=db,
        filter=itemModifier_map,
    )

    return await _crud.CRUD_itemModifier.update(
