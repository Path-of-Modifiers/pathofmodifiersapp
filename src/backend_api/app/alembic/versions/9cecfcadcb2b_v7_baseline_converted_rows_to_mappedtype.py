"""v7_baseline_converted_rows_to_mappedtype

Revision ID: 9cecfcadcb2b
Revises: 
Create Date: 2024-09-26 06:36:58.218402
...
def upgrade() -> None:
    ...
    sa.Column('currencyId', sa.BigInteger(), sa.Identity(always=False), nullable=False),
    ...
    sa.Column('createdAt', sa.DateTime(), nullable=False),
    sa.Column('updatedAt', sa.DateTime(), nullable=True),
    sa.CheckConstraint("username ~* '^[[:alnum:]]+$'", name='check_username_letters_and_numbers'),
    ...
    sa.Column('itemId', sa.BigInteger(), sa.Identity(always=False), nullable=False),
    ...
    op.create_index(op.f('ix_item_modifier_orderId'), 'item_modifier', ['orderId'], unique=False)
...
def downgrade() -> None:
    ...
    op.drop_index(op.f('ix_item_modifier_orderId'), table_name='item_modifier')
