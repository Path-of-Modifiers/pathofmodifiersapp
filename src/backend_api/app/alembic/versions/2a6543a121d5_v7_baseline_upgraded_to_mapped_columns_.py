"""v7 baseline upgraded to mapped columns for scalars during async

Revision ID: 2a6543a121d5
Revises: 
Create Date: 2024-09-23 07:58:43.108367
...
def upgrade() -> None:
    ...
    op.create_table(
        "currency",
        sa.Column(
            "currencyId", sa.BigInteger(), sa.Identity(always=False), nullable=False
        ),
    ...
    op.create_table(
        ...
        sa.Column("updatedAt", sa.DateTime(), nullable=True),
        sa.CheckConstraint(
            "username ~* '^[[:alnum:]]+$'", name="check_username_letters_and_numbers"
        ),
    ...
    op.create_index(
        op.f("ix_item_modifier_orderId"), "item_modifier", ["orderId"], unique=False
    )
    op.create_index(
