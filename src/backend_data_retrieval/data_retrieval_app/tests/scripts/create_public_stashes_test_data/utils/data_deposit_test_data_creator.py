...
class DataDepositTestDataCreator:
    def __init__(self, n_of_items: int) -> None:
        ...
        self.db_modifier_df = self._get_df_from_url("modifier/")
    ...
    def _find_connected_modifier_ids_and_rolls(
        ...
        """
        Example output connected_modifier_ids_dict dict:
        {
            3: [
                [1],
                [5, 6]
        ...
        def get_modifier_ids_and_rolls(
            ...
            return pd.Series({"modifier_ids": choosable_modifier_ids, "rolls": rolls})
        ...
        for key in modifier_distrubution:
            ...
            # connected_modifier_df.apply(get_modifier_ids_and_rolls, axis=1)
            ...
            modifier_ids_and_rolls = connected_grouped_modifier_df.apply(
                get_modifier_ids_and_rolls, axis=1
            )

            modifier_ids = modifier_ids_and_rolls["modifier_ids"].to_list()
            connected_modifier_ids_dict[key] = modifier_ids

            rolls = modifier_ids_and_rolls["rolls"].to_list()
    ...
    def _create_effect(self, effect: str, rolls: list[str | list[float]]):
        ...
        for i, roll in enumerate(rolls):
            ...
            if isinstance(roll, str):
                ...
                new_effect += f"{random_int(roll[0], roll[1])}"
    ...
    def _choose_and_make_modifiers(
        ...
        if not all(len(list_to_compare) == the_len for list_to_compare in it):
            raise ValueError(
                "can_duplicate, distrubution.keys(), modifier_ids_to_choose.keys() "
                / ", roll_ranges.keys() do not have the same length!"
            )
        ...
        for i, key in enumerate(distrubution):
            ...
            for _ in range(n_modifiers_to_create):
                ...
                effect: str = self.db_modifier_df.loc[
                    self.db_modifier_df["modifierId"] == chosen_modifier_ids[0],
                    "effect",
                ].tolist()[0]
