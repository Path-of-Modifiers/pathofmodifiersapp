...
class APIHandler:
        ...
        try:
            while True:
                for i in range(expedition_depth):
                    futures = [
                        executor.submit(
                            self._run_async_send_request(
                                session, waiting_for_next_id_lock
                            )
                        )
                        for j in range(executor._max_workers)
                    ]
                    stashes = []
    ...
    def dump_stream(
        ...
        with ThreadPoolExecutor(max_workers=listeners + 1) as executor:
            executor.submit
            asyncio.run(
                self._follow_stream(
                    executor, stashes_ready_event, waiting_for_next_id_lock, stash_lock
                )
            )
