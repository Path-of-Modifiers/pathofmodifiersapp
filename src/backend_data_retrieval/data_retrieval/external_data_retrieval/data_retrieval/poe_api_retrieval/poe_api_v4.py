        ...
        try:
            while True:
                while self.requests_since_last_checkpoint < expedition_depth:
                    ...
                    print(
                        "Locked the next change id, waiting for a new one before releasing."
                    )
                    async with session.get(
                        ...
                        stash_lock.acquire()
                        self.stashes += response_json["stashes"]
                        stash_lock.release()
                        del response_json
        ...
        for i in range(listeners):
            following_stream_threads.append(
                threading.Thread(
                    ...
                    args=(stashes_ready_event, waiting_for_next_id_lock, stash_lock),
                )
            )
        print("Starting follow stream threads")
        for thread in following_stream_threads:
            thread.start()
