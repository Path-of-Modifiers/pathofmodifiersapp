...
class APIHandler:
    ...
    async def _follow_stream(
        ...
        while self.requests_since_last_checkpoint < expedition_depth:
            print(f"Thread {threading.get_ident()} starting 5 requests")
            stashes = await self._send_n_recursion_requests(
    ...
    async def _async_follow_stream(
        self,
        ...
        print(f"Thread {threading.get_ident()} starting to follow the stream.")
        try:
            while True:
                ...
                #     )
                # )
                # the_task = asyncio.create_task(asyncio.wait(1))
                # await asyncio.wait((the_task,))
                future = asyncio.run_coroutine_threadsafe(
                    self._follow_stream(session, waiting_for_next_id_lock, stash_lock),
                    loop,
                )
                future.result()
    ...
    def _run_async_follow_stream(
        ...
        asyncio.run(
            self._async_follow_stream(
                loop, stashes_ready_event, waiting_for_next_id_lock, stash_lock
            )
        )
    ...
    def dump_stream(
        ...
        following_stream_threads = []
        thread_loops = []
        for i in range(listeners):
            loop = asyncio.new_event_loop()
            thread_loops.append(loop)
            following_stream_threads.append(
                threading.Thread(
                    target=self._run_async_follow_stream,
                    args=(
                        loop,
                        stashes_ready_event,
                        waiting_for_next_id_lock,
        ...
        print("Starting follow stream threads")
        for thread, loop in zip(following_stream_threads, thread_loops):
            thread.start()
