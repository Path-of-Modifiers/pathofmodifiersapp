          ...
          (labelPart, labelIndex, splitSelectedModifer) => {
            ...
            return labelPartSplit.map((word, wordIndex) => {
              if (wordIndex < labelPartSplit.length - 1 || !isNotEnd) {
                ...
                const modifierId = selectedModifier.modifierId;
                const position =
                  selectedModifier.groupedModifierProperties.position[
                    labelIndex
                  ];
                return (
                  <HStack
                  ...
                  >
                    <Text verticalAlign="center">{word}</Text>
                    <FancyModifierInput
                      currentlyTakingInput={currentlyTakingInput[labelIndex]}
                      modifierId={modifierId}
                      position={position}
                      selectedModifierIndex={props.index}
                      textRolls={textRolls}
                      orderIndex={labelIndex}
                      changeTakingInput={changeTakingInput}
                    />
                  </HStack>
                );
              }
