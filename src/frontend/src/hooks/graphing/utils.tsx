...
function meanOfValues(values: number[], kwargs?: meanOfValuesKWARGS): number {
  ...
  if (kwargs !== undefined) {
    ...
    const normalValues = valuesWithinNStandardDeviations(
      values,
      kwargs.nStandardDeviations
    );
    return (
      normalValues.reduce((acc, val) => acc + val, 0) / normalValues.length
    );
  } else {
...
const valuesWithinNStandardDeviations = (values: number[], N: number) => {
  ...
  const normalValues = values.filter(
    (value) => Math.abs(value - mean) < N * std
  );
...
export const groupByAndMeanTopN = (values: Datum[], topN: number) => {
  const grouped_values: Datum[] = _(values)
    ...
    .map((value, key) => ({
      ...
      valueInChaos: meanOfValues(_.map(value, "valueInChaos"), {
        topN: topN,
        normalValuesOnly: true,
        nStandardDeviations: 2,
      }),
    }))
    .value()
    .slice(0, -1);
