...
async def get_currency(currencyName: int, db: "Session"):
    currency = db.query(_models.Currency).filter(_models.Currency.currencyName == currencyName).first()
    return currency
...
async def update_currency(currencyData: _schemas.CreateCurrency, currency: _models.Currency, db: "Session"
) -> _schemas.Currency:
...
async def update_item(itemData: _schemas.CreateItem, item: _models.Item, db: "Session"
) -> _schemas.Item:
...
async def get_transaction(transactionId: int, db: "Session"):
    transaction = db.query(_models.Transaction).filter(_models.Transaction.transactionId == transactionId).first()
    return transaction
...
async def update_transaction(transactionData: _schemas.CreateTransaction, transaction: _models.Transaction, db: "Session"
) -> _schemas.Transaction:
...
async def get_item_base_type(item_base_type_id: int, db: "Session"):
    item_base_type = db.query(_models.ItemBaseType).filter(_models.ItemBaseType.baseType == item_base_type_id).first()
...
async def update_stash(stashData: _schemas.CreateStash, stash: _models.Stash, db: "Session"
) -> _schemas.Stash:
...
async def update_account(accountData: _schemas.CreateAccount, account: _models.Account, db: "Session"
) -> _schemas.Account:
...
async def update_X(XData: _schemas.CreateX, X: _models.X, db: "Session"
) -> _schemas.X:
    X.name = XData.name
